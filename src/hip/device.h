...
#ifdef ZFP_DEBUG
  if (!success) {
    std::cerr << "zfp::hip : failed to allocate device memory";
    if (what)
      std::cerr << " for " << what;
    std::cerr << std::endl;
  }
...
#ifdef ZFP_DEBUG
    std::cerr << "zfp::hip : failed to copy " << (what ? what : "data") << " from host to device" << std::endl;
#endif
...
{
  bool success = (hipMemcpy(h_pointer, *d_pointer, size, hipMemcpyDeviceToHost) == hipSuccess);
#ifdef ZFP_DEBUG
  if (!success)
    std::cerr << "zfp::hip : failed to copy " << (what ? what : "data") << " from device to host" << std::endl;
#endif
  device_free(*d_pointer);
  *d_pointer = NULL;
