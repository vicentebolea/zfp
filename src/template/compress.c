...
_t2(compress, Scalar, 1)(zfp_stream* stream, const zfp_field* field)
{
  ...
  for (x = 0; x < mx; x += 4, data += 4) {
    uint16 block_size = (uint16)_t2(zfp_encode_block, Scalar, 1)(stream, data);
    if (length_table)
      *length_table++ = block_size;
  }
  if (x < nx) {
    uint16 block_size = (uint16)_t2(zfp_encode_partial_block_strided, Scalar, 1)(stream, data, nx - x, 1);
    if (length_table)
      *length_table++ = block_size;
  }
}
...
_t2(compress_strided, Scalar, 1)(zfp_stream* stream, const zfp_field* field)
{
  ...
  for (x = 0; x < nx; x += 4) {
    ...
    uint16 block_size;
    if (nx - x < 4)
      block_size = (uint16)_t2(zfp_encode_partial_block_strided, Scalar, 1)(stream, p, nx - x, sx);
    else
      block_size = (uint16)_t2(zfp_encode_block_strided, Scalar, 1)(stream, p, sx);
    if (length_table)
      *length_table++ = block_size;
  }
}
...
_t2(compress_strided, Scalar, 2)(zfp_stream* stream, const zfp_field* field)
{
  ...
  for (y = 0; y < ny; y += 4)
    for (x = 0; x < nx; x += 4) {
      ...
      uint16 block_size;
      if (nx - x < 4 || ny - y < 4)
        block_size = (uint16)_t2(zfp_encode_partial_block_strided, Scalar, 2)(stream, p, MIN(nx - x, 4u), MIN(ny - y, 4u), sx, sy);
      else
        block_size = (uint16)_t2(zfp_encode_block_strided, Scalar, 2)(stream, p, sx, sy);
      if (length_table)
        *length_table++ = block_size;
    }
}
    ...
    for (y = 0; y < ny; y += 4)
      for (x = 0; x < nx; x += 4) {
        ...
        if (nx - x < 4 || ny - y < 4 || nz - z < 4)
          block_size = (uint16)_t2(zfp_encode_partial_block_strided, Scalar, 3)(stream, p, MIN(nx - x, 4u), MIN(ny - y, 4u), MIN(nz - z, 4u), sx, sy, sz);
        else
          block_size = (uint16)_t2(zfp_encode_block_strided, Scalar, 3)(stream, p, sx, sy, sz);
        if (length_table)
          *length_table++ = block_size;
      }
}
      ...
      for (y = 0; y < ny; y += 4)
        for (x = 0; x < nx; x += 4) {
          ...
          if (nx - x < 4 || ny - y < 4 || nz - z < 4 || nw - w < 4)
            block_size = (uint16)_t2(zfp_encode_partial_block_strided, Scalar, 4)(stream, p, MIN(nx - x, 4u), MIN(ny - y, 4u), MIN(nz - z, 4u), MIN(nw - w, 4u), sx, sy, sz, sw);
          else
            block_size = (uint16)_t2(zfp_encode_block_strided, Scalar, 4)(stream, p, sx, sy, sz, sw);
          if (length_table)
            *length_table++ = block_size;
        }
}
